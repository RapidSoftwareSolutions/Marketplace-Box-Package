...
$app->post('/api/Box/deleteFolder', function ($request, $response) {
    ...
    if(!empty($post_data['args']['recursive']))
    {
        $data['recursive'] = $post_data['args']['recursive'];
    }
    ...
    try {
        ...
        if(in_array($resp->getStatusCode(), ['200', '201', '202', '203', '204'])) {
            ...
            if(empty($result['contextWrites']['to'])) {
                $result['contextWrites']['to']['status_msg'] = "Directory successfully deleted";
            }
        } else {
