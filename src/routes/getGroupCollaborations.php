...
$app->post('/api/Box/getGroupCollaborations', function ($request, $response) {
    ...
    if(!empty($post_data['args']['offset'])){
        $fields['offset'] = $post_data['args']['offset'];
    }
    if(!empty($post_data['args']['limit'])){
        $fields['limit'] = $post_data['args']['limit'];
    }

    $query_str = $settings['default_url'] . "groups/$groupId/collaborations";
